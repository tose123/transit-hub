...
func TestEmbedWorkspaceIndexHelpers(t *testing.T) {
	if got := embedSessionKey("token-1"); got != "tickets:embed:session:token-1" {
		t.Fatalf("embed session key = %q", got)
	}
	if got := embedWorkspaceIndexKey(" user-1 ", " account-1 "); got != "tickets:embed:workspace:user-1:account-1" {
