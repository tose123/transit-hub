...
import { Layers, Loader2, X, Save, CheckCircle2, CircleHelp, Zap, Settings2 } from 'lucide-vue-next'
...
const exclusiveLabel = (isExclusive: boolean): string => {
  return isExclusive ? t('admin.dashboard.groupList.exclusiveLabels.exclusive') : t('admin.dashboard.groupList.exclusiveLabels.public')
}

const statusLabel = (status: string): string => {
  const key = `admin.dashboard.groupList.statusLabels.${status}`
  const result = t(key)
...
const saveMappingsData = async () => {
  ...
  try {
    ...
    error.value = err instanceof Error ? err.message : 'admin.dashboard.groupList.saveError'
  } finally {
...
const loadData = async () => {
  ...
  try {
    ...
    error.value = err instanceof Error ? err.message : 'admin.dashboard.groupList.loadError'
  } finally {
    loading.value = false
  }
}
  ...
  <Teleport defer to="body">
    <div v-if="open" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div
        role="dialog"
        aria-modal="true"
        class="relative w-full max-w-[90rem] overflow-hidden rounded-[2rem] border border-border/60 bg-card text-card-foreground shadow-2xl shadow-primary/10 animate-in fade-in zoom-in-95 duration-200"
      >
        ...
        <div class="flex items-start justify-between gap-4 px-6 pt-6">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h2 class="text-lg font-semibold text-foreground">{{ t('admin.dashboard.groupList.title') }}</h2>
              <p class="text-sm text-muted-foreground">
                {{ t('admin.dashboard.groupList.subtitle', { count: totalMappings }) }}
              </p>
            </div>
          </div>
          <div class="flex items-center gap-2">
            ...
            >
              ...
              {{ showSaveSuccess ? t('admin.dashboard.groupList.saveSuccess') : (isSaving ? t('admin.dashboard.groupList.saving') : t('admin.dashboard.groupList.save')) }}
            </button>
            <button
              type="button"
              class="rounded-md p-1 text-muted-foreground transition-colors hover:bg-surface-elevated hover:text-foreground"
              :title="t('admin.dashboard.groupList.close')"
              @click="emit('close')"
            >
              <X class="h-5 w-5" />
            </button>
        ...
        <div class="px-6 py-6">
          ...
          >
            ...
            <p class="text-sm text-muted-foreground">{{ t('admin.dashboard.groupList.empty') }}</p>
          </div>
            ...
            <table class="w-full text-sm">
              <thead class="sticky top-0 z-10 bg-surface/90 backdrop-blur">
                <tr class="border-b border-border/60 text-left text-xs font-medium text-muted-foreground">
                  <th class="px-4 py-3">{{ t('admin.dashboard.groupList.columns.index') }}</th>
                  <th class="px-4 py-3">{{ t('admin.dashboard.groupList.columns.ownGroup') }}</th>
                  <th class="px-4 py-3">{{ t('admin.dashboard.groupList.columns.platform') }}</th>
                  <th class="px-4 py-3 min-w-[6rem]">{{ t('admin.dashboard.groupList.columns.groupType') }}</th>
                  <th class="px-4 py-3 min-w-[5rem]">{{ t('admin.dashboard.groupList.columns.status') }}</th>
                  <th class="px-4 py-3 min-w-[6rem]">{{ t('admin.dashboard.groupList.columns.ownMultiplier') }}</th>
                  <th class="px-4 py-3">{{ t('admin.dashboard.groupList.columns.upstreamGroup') }}</th>
                  <th class="px-4 py-3 min-w-[6rem]">{{ t('admin.dashboard.groupList.columns.upstreamMultiplier') }}</th>
                  <th class="px-4 py-3 min-w-[12rem]">
                    <span class="inline-flex items-center gap-1">
                      {{ t('admin.dashboard.groupList.columns.autoPricing') }}
                  ...
                  <td class="px-4 py-3 align-middle transition-colors" :class="{'bg-primary/5': hoveredMappingIndex === mappingIndex}">
                    <div class="flex items-center gap-2">
                      ...
                      >
                        ...
                        {{ t(`admin.dashboard.groupList.autoPricingStatus.${
                      ...
                      >
                        ...
                        {{ getAutoPricingStatus(mapping.ownGroup) === 'not_configured' ? t('admin.dashboard.groupList.autoPricingActions.configure') : t('admin.dashboard.groupList.autoPricingActions.edit') }}
                      </button>
                    </div>
                  </td>
                </tr>
  ...
  <Teleport to="body">
    ...
    >
      <div
        ...
        :style="tipStyle"
      >
        {{ t('admin.dashboard.groupList.autoPricingTip') }}
        <span
